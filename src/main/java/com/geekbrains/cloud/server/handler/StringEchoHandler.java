...
import com.geekbrains.cloud.server.service.UserNameService;
...
public class StringEchoHandler extends SimpleChannelInboundHandler<String> {
    ...
    public StringEchoHandler (
        ...
        this.users = users;
            ...
            if (userNameService.getUserName().equals("user_unconnected")) {
                log.debug("Превышено допустимое количество пользователей на сервере!");
                users.remove();
        }
    }
